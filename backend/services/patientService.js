...
async function insertPatientData(requestBody) {
  const { Items } = await patientRepository.getPatientByEmail(
    requestBody.email
  );
...
async function updatePatientData(patientId, newPatientData) {
  ...
  const newEmailUpdateValue = newPatientData.find(
    (newValue) => newValue.updateKey === "email"
  );

  if (newEmailUpdateValue?.updateKey === "email") {
    const { Items } = await patientRepository.getPatientByEmail(
      newEmailUpdateValue.updateValue
    );
